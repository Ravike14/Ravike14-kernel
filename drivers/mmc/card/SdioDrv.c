			...
			unsigned int bMore)
{
	...
	/*if(ENABLE_IRQ_LOG) printk("sdioDrv_WriteAsync  iStatus:%d                           ------\n",iStatus);
...
//we don't support multi-block mode now
	...
	if (gWiFi_SDIOBlockMode) {
		...
		if (transferSize > 1) {
			status=SDIO_SyncReadBlockF(out_data_buffer, in_source_address, 1,
					transferSize, SDIO_MAX_PACKET_LEN, FUNCTION_2, incr_fix, 0);
...
//we don't suport block mode now
	...
	if(gWiFi_SDIOBlockMode)
	{
		...
		if (transferSize > 1)
		{
			status=SDIO_SyncWriteBlockF(in_data_buffer,out_target_address, 1,
					transferSize, SDIO_MAX_PACKET_LEN, FUNCTION_2, incr_fix, 0);
			if(status!=SDIO_SUCCESS)
			{

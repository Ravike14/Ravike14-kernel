...
retry:
	...
	if (rc < 0) {
		pr_err("[CAM]mt9d015_i2c_read 0x%x failed!\n", raddr);
...
retry:
	...
	if (rc < 0) {
		pr_err("[CAM]i2c_write_b failed, addr = 0x%x, val = 0x%x!\n",
			waddr, wdata);
		pr_err("[CAM]starting write_w retry policy count:%d\n", count);
		udelay(10);
		count++;
		if (count < 20) {
			if (count > 10)
...
retry:
	...
	if (rc < 0) {
		pr_err("[CAM]i2c_write_b failed, addr = 0x%x, val = 0x%x!\n",
			waddr, bdata);
		pr_err("[CAM]starting write_b retry policy count:%d\n", count);
		udelay(10);
		count++;
		if (count < 20) {
			if (count > 10)
...
static int mt9d015_i2c_read_fuseid(struct sensor_cfg_data *cdata)
{
	...
	cdata->cfg.fuse.fuse_id_word3 =
		(fuseid[3]<<16)|(fuseid[2]);
	cdata->cfg.fuse.fuse_id_word4 =
		(fuseid[1]<<16)|(fuseid[0]);
	...
	if (update_type == REG_INIT) {
		...
		if (!CSI_CONFIG) {
			...
			mt9d015_csi_params.hs_impedence = 0x0F;
					v4l2_subdev_notify(&(mt9d015_s_ctrl.sensor_v4l2_subdev),
			NOTIFY_CSIC_CFG,
			&mt9d015_csi_params);
			msleep(50);
			CSI_CONFIG = 1;
...
static int mt9d015_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	if (!rc) {
		...
		pr_info("[CAM] gpio_request sensor_reset pin failed !!!\n");

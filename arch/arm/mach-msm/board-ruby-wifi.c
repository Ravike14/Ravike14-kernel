...
#include "board-ruby-wifi.h"

int ruby_wifi_power(int on);
int ruby_wifi_reset(int on);
int ruby_wifi_set_carddetect(int on);
int ruby_wifi_get_mac_addr(unsigned char *buf);
...
static struct wifi_platform_data ruby_wifi_control = {
	...
	.get_mac_addr	= ruby_wifi_get_mac_addr,
};
...
static unsigned ruby_wifi_update_nvs(char *str)
{
...
int ruby_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"ruby_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	ruby_wifi_update_nvs("sd_oobonly=1\n");
#endif
	ruby_wifi_update_nvs("btc_params80=0\n");
	ruby_wifi_update_nvs("btc_params6=30\n");
	ruby_init_wifi_mem();

...
/* HTC: rename MODULE_NAME to discriminate the al and al_bootloader */
#ifdef MODULE_NAME
#undef MODULE_NAME
#define MODULE_NAME "[AL_DLD]"
#endif
...
/* ++SSD_RIL: Mars@20110615: set debug file permission to 644 */
					/* S_IRUGO | S_IWUGO, */
					S_IRUGO | S_IWUSR,
/* --SSD_RIL */
					sdio_dld_debug.sdio_dld_debug_root,
...
static void sdio_dld_set_op_mode(enum sdio_dld_op_mode op_mode)
{
	...
	case SDIO_DLD_NORMAL_MODE:
		memcpy(sdio_dld->op_mode_name,
			   SDIO_DLD_NORMAL_MODE_NAME, sizeof(SDIO_DLD_NORMAL_MODE_NAME));
		break;
	case SDIO_DLD_BOOT_TEST_MODE:
		memcpy(sdio_dld->op_mode_name,
			   SDIO_DLD_BOOT_TEST_MODE_NAME, sizeof(SDIO_DLD_BOOT_TEST_MODE_NAME));
		break;
	case SDIO_DLD_AMSS_TEST_MODE:
		memcpy(sdio_dld->op_mode_name,
			   SDIO_DLD_AMSS_TEST_MODE_NAME, sizeof(SDIO_DLD_AMSS_TEST_MODE_NAME));
		break;
	default:
				   ...
				   int size_to_write)
{
	...
	int size_to_add; /* HTC: potential fix */
	...
	if (!dest_wr_ptr) {
		pr_err(MODULE_NAME ": %s - param ""dest_wr_ptr"" is NULL.\n",
			   __func__);
		return -EINVAL;
	} else /* HTC: potential fix */
		size_to_add = *dest_wr_ptr;

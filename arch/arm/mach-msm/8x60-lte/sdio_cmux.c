...
#include <mach/sdiodbg-8x60.h> /* HTC */

/* HTC */
extern unsigned int sdio_dbg_flag;
#define IS_CMUX_DBG (sdio_dbg_flag & DBG_CMUX)
...
/* ++SSD_RIL: Mars@20110719: keep ch alloc status */
	int is_ch_alloced;
/* --SSD_RIL */
...
#if defined(DEBUG) /* HTC: add IS_CMUX_DBG checking */
#define D_DUMP_BUFFER(prestr, cnt, buf) \
do { \
	if (msm_sdio_cmux_debug_mask & MSM_SDIO_CMUX_DUMP_BUFFER || IS_CMUX_DBG) { \
		int i; \
		pr_debug("%s", prestr); \
		for (i = 0; i < cnt; i++) \
			pr_info("%.2x", buf[i]); \
...
#define D(x...) \
do { \
	if (msm_sdio_cmux_debug_mask & MSM_SDIO_CMUX_DEBUG || IS_CMUX_DBG) \
		pr_debug(x); \
} while (0)
...
/* ++SSD_RIL: Mars@20110719: keep ch alloc status */
		if (logical_ch[id].is_ch_alloced == 0) {
			pr_err("%s: OPEN: Remote Ch%d not alloc\n", __func__, id);
			return -ENODEV;
		}
/* --SSD_RIL */
...
/* ++SSD_RIL: Mars@20110719: keep ch alloc status */
		if (logical_ch[id].is_ch_alloced == 0) {
			pr_err("%s: CLOSE: Remote Ch%d not alloc\n", __func__, id);
			return -ENODEV;
		}
/* --SSD_RIL */
...
/* ++SSD_RIL: Mars@20110719: keep ch alloc status */
		if (logical_ch[id].is_ch_alloced == 0) {
			pr_err("%s: DATA: Remote Ch%d not alloc\n", __func__, id);
			return -ENODEV;
		}
/* --SSD_RIL */
...
/* ++SSD_RIL: Mars@20110719: keep ch alloc status */
		if (logical_ch[id].is_ch_alloced == 0) {
			pr_err("%s: STATUS: Remote Ch%d not alloc\n", __func__, id);
			return -ENODEV;
		}
/* --SSD_RIL */
...
/* ++SSD_RIL: Mars_Lin@20110628: Check ch->priv brfore call sdio_ctl_write_done */
		/* if (ch->write_done) */
		if (ch->write_done && ch->priv)
/* --SSD_RIL */
...
static int sdio_cmux_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sdio_cmux_wq) || !sdio_cmux_wq) {
	...
	if (IS_ERR(sdio_cdemux_wq) || !sdio_cdemux_wq) {
		pr_err("%s: create_singlethread_workqueue() ENOMEM\n",
			__func__);
		r = -ENOMEM;
		goto error1;
